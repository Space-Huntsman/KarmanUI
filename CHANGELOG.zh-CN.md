...
- Calendar
  - 导入 kui-button 和 kui-button-group (#17376 by @masongzhi)
- MessageBox
...
- Theme
  - 增加 Chrome 插件: Karman Theme Extension (#16686 by @iamkun)
- Icon
...
- NavMenu
  - 修复点击 kui-submenu 多次触发 childMenu 问题 (#14443 by @PanJiaChen)
- Dropdown
...
- DropdownItem
  - `kui-dropdown-item` 支持添加 icon (by @gabrielboliveira in #14088)
- Input
...
- Menu
  - 支持 `kui-menu-item` 不添加 index (by @georgyfarniev in #13298)
- Table
...
- Docs
  ...
  - 添加 `kui-input` 是受控组件的警示 (by @wacky6 in #14463)
...
- Build
  - 压缩 Karman 文档站的 JS 和 CSS 文件 (by @iamkun in #14430)
...
#### 修复
- 修复升级 Webpack 4 产生的问题，无法具名 `import` 组件，`KARMAN.locale()` 调用报错。（by @island205 in #14220）
...
- MessageBox
  - 修复连续打开两个 MessageBox 时 `kui-popup-parent--hidden` 无法移除的问题，#11371
...
- 通过指令调用的 Loading 现在支持以 `karman-loading-custom-class` 属性的方式设置自定义类名，#8826（by @earlymeme）
...
- 综合
  ...
  - 新增全局配置组件尺寸的功能。在引入 Karman 时，配置 `size` 字段可以改变所有组件的默认尺寸
- Button
