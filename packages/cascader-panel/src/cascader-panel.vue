...
const getSibling = (el, distance) => {
  const { parentNode } = el;
  if (parentNode) {
    const siblings = parentNode.querySelectorAll('.kui-cascader-node[tabindex="-1"]');
  ...
  methods: {
    ...
    handleKeyDown(e) {
      ...
      switch (keyCode) {
        ...
        case KeyCode.left:
          ...
          if (preMenu) {
            const expandedNode = preMenu.$el.querySelector('.kui-cascader-node[aria-expanded="true"]');
        ...
        case KeyCode.right:
          ...
          if (nextMenu) {
            const firstNode = nextMenu.$el.querySelector('.kui-cascader-node[tabindex="-1"]');
            focusNode(firstNode);
          }
          break;
        case KeyCode.enter:
    ...
    */
    ...
    scrollIntoView() {
      ...
      menus.forEach(menu => {
        const menuKarman = menu.$el;
        if (menuKarman) {
          const container = menuKarman.querySelector('.kui-scrollbar__wrap');
          const activeNode = menuKarman.querySelector('.kui-cascader-node.is-active') ||
            menuKarman.querySelector('.kui-cascader-node.in-active-path');
