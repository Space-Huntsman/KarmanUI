<template>
  <div
    ...
    :class="[
      'kui-cascader',
      realSize && `kui-cascader--${realSize}`,
    ...
    <kui-input
      ...
      <template slot="suffix">
        <i
          ...
          key="clear"
          class="kui-input__icon kui-icon-circle-close"
    ...
    <div v-if="multiple" class="kui-cascader__tags">
      <kui-tag
      ...
      <input
        ...
        type="text"
        class="kui-cascader__search-input"
    ...
    <transition name="kui-zoom-in-top" @after-leave="handleDropdownLeave">
      <div
        v-show="dropDownVisible"
        ref="popper"
        :class="['kui-popper', 'kui-cascader__dropdown', popperClass]">
        <kui-cascader-panel
          ref="panel"
          v-show="!filtering"
          ...
          @close="toggleDropDownVisible(false)"></kui-cascader-panel>
        <kui-scrollbar
          ref="suggestionPanel"
          v-if="filterable"
          v-show="filtering"
          tag="ul"
          class="kui-cascader__suggestion-panel"
          view-class="kui-cascader__suggestion-list"
          ...
          <template v-if="suggestions.length">
            <li
              ...
              :key="item.uid"
              :class="[
                'kui-cascader__suggestion-item',
                item.checked && 'is-checked'
              ]"
              :tabindex="-1"
              @click="handleSuggestionClick(index)">
              <span>{{ item.text }}</span>
              <i v-if="item.checked" class="kui-icon-check"></i>
            </li>
          </template>
...
import KuiCascaderPanel from 'karman-ui/packages/cascader-panel';
import AriaUtils from 'karman-ui/src/utils/aria-utils';
import { t } from 'karman-ui/src/locale';
import { isEqual, isEmpty, kebabCase } from 'karman-ui/src/utils/util';
import { isUndefined, isFunction } from 'karman-ui/src/utils/types';
import { isDef } from 'karman-ui/src/utils/shared';
...
export default {
  ...
  computed: {
    realSize() {
      ...
      return this.size || _elFormItemSize || (this.$KARMAN || {}).size;
    },
    tagSize() {
  ...
  methods: {
    ...
    focusFirstNode() {
      this.$nextTick(() => {
        ...
        if (filtering && suggestionPanel) {
          firstNode = suggestionPanel.$el.querySelector('.kui-cascader__suggestion-item');
        } else {
          const firstMenu = popper.querySelector('.kui-cascader-menu');
          firstNode = firstMenu.querySelector('.kui-cascader-node[tabindex="-1"]');
        }
    ...
    handleSuggestionKeyDown(event) {
      ...
      switch (keyCode) {
        case KeyCode.enter:
          ...
          break;
        case KeyCode.up:
          const prev = target.previousKarmanSibling;
          prev && prev.focus();
          break;
        case KeyCode.down:
          const next = target.nextKarmanSibling;
          next && next.focus();
          break;
        case KeyCode.esc:
        case KeyCode.tab:
    ...
    updateStyle() {
      ...
      const inputInner = $el.querySelector('.kui-input__inner');

      if (!inputInner) return;

      const tags = $el.querySelector('.kui-cascader__tags');
      let suggestionPanelEl = null;

      if (suggestionPanel && (suggestionPanelEl = suggestionPanel.$el)) {
        const suggestionList = suggestionPanelEl.querySelector('.kui-cascader__suggestion-list');
