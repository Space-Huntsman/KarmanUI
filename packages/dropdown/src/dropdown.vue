<script>
  ...
  export default {
    ...
    watch: {
      visible(val) {
        this.broadcast('KuiDropdownMenu', 'visible', val);
        this.$emit('visible-change', val);
      },
      focusing(val) {
        const selfDefine = this.$el.querySelector('.kui-dropdown-selfdefine');
        if (selfDefine) { // 自定义
          if (val) {
    ...
    methods: {
      ...
      initAria() {
        ...
        if (!this.splitButton) { // 自定义
          ...
          this.triggerElm.setAttribute('class', (this.triggerElm.getAttribute('class') || '') + ' kui-dropdown-selfdefine'); // 控制
        }
      },
      initEvent() {
    ...
    render(h) {
      ...
      let triggerElm = null;
      if (splitButton) {
        triggerElm = <kui-button-group>
          <kui-button type={type} size={dropdownSize} nativeOn-click={handleMainButtonClick} disabled={disabled}>
            {this.$slots.default}
          </kui-button>
          <kui-button ref="trigger" type={type} size={dropdownSize} class="kui-dropdown__caret-button" disabled={disabled}>
            <i class="kui-dropdown__icon kui-icon-arrow-down"></i>
          </kui-button>
        </kui-button-group>;
      } else {
      ...
      return (
        <div class="kui-dropdown" v-clickoutside={hide} aria-disabled={disabled}>
          {triggerElm}
          {menuElm}
        </div>
      );
