<script>
  ...
  export default {
    ...
    computed: {
      ...
      menuTransitionName() {
        return this.rootMenu.collapse ? 'kui-zoom-in-left' : 'kui-zoom-in-top';
      },
      opened() {
      ...
      isFirstLevel() {
        ...
        while (parent && parent !== this.rootMenu) {
          if (['KuiSubmenu', 'KuiMenuItemGroup'].indexOf(parent.$options.componentName) > -1) {
            isFirstLevel = false;
            break;
          } else {
    ...
    methods: {
      ...
      handleClick() {
        ...
        this.dispatch('KuiMenu', 'submenu-click', this);
      },
      handleMouseenter(event, showTimeout = this.showTimeout) {
        ...
        this.dispatch('KuiSubmenu', 'mouse-enter-child');
      ...
      handleMouseleave(deepDispatch = false) {
        ...
        this.dispatch('KuiSubmenu', 'mouse-leave-child');
        clearTimeout(this.timeout);
        this.timeout = setTimeout(() => {
        ...
        if (this.appendToBody && deepDispatch) {
          if (this.$parent.$options.name === 'KuiSubmenu') {
    ...
    render(h) {
      ...
      const popupMenu = (
        <transition name={menuTransitionName}>
          <div
            ref="menu"
            v-show={opened}
            class={[`kui-menu--${mode}`, popperClass]}
            ...
            <ul
              role="menu"
              class={['kui-menu kui-menu--popup', `kui-menu--popup-${currentPlacement}`]}
      ...
      const inlineMenu = (
        <kui-collapse-transition>
          <ul
            role="menu"
            class="kui-menu kui-menu--inline"
            v-show={opened}
            style={{ backgroundColor: rootMenu.backgroundColor || '' }}>
            {$slots.default}
          </ul>
      ...
      ) ? 'kui-icon-arrow-down' : 'kui-icon-arrow-right';
      ...
      return (
        ...
        >
          ...
          >
            {$slots.title}
            <i class={[ 'kui-submenu__icon-arrow', submenuTitleIcon ]}></i>
          </div>
