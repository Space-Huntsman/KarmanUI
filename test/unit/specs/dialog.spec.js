...
describe('Dialog', () => {
  ...
  it('create', done => {
    vm = createVue({
      template: `
        <div>
          <kui-dialog :title="title" :visible="visible"></kui-dialog>
        </div>
      `,
    ...
    setTimeout(() => {
      ...
      expect(vm.$el.querySelector('.kui-dialog__title').textContent).to.equal('dialog test');
  ...
  it('render correct content', done => {
    vm = createVue({
      template: `
        <div>
          <kui-dialog :title="title" :visible="visible">
            <span>这是一段信息</span>
    ...
    setTimeout(() => {
      const footerBtns = vm.$el.querySelectorAll('.kui-dialog__footer .kui-button');
      expect(vm.$el.querySelector('.kui-dialog__body span').textContent).to.equal('这是一段信息');
  ...
  it('append to body', done => {
    vm = createVue({
      template: `
        <div>
          <kui-dialog :title="title" append-to-body :visible="visible"></kui-dialog>
        </div>
      `,
  ...
  it('open and close', done => {
    vm = createVue({
      template: `
        <div>
          <kui-dialog :title="title" :visible.sync="visible">
            <span>这是一段信息</span>
          </kui-dialog>
        </div>
      `,
  ...
  describe('props', () => {
    const getDialogVm = (props, options) => {
      return createVue(Object.assign({
        template: `
          <div>
            <kui-dialog ${ props } :title="title" :visible="visible">
              <span>这是一段信息</span>
            </kui-dialog>
          </div>
        `,
    ...
    it('fullscreen', () => {
      ...
      const dialogEl = vm.$el.querySelector('.kui-dialog');
    ...
    it('top', () => {
      ...
      expect(vm.$el.querySelector('.kui-dialog').style.marginTop).to.equal('100px');
    });

    it('custom-class', () => {
      vm = getDialogVm('custom-class="my-dialog"');
      expect(vm.$el.querySelector('.kui-dialog').classList.contains('my-dialog')).to.true;
    });
  });
  ...
  it('click header btn', done => {
    ...
    const dialog = vm.$children[0];
    setTimeout(() => {
      dialog.$el.querySelector('.kui-dialog__headerbtn').click();
      setTimeout(() => {
        expect(vm.visible).to.be.false;
        done();
      }, 500);
  ...
  it('before close', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <kui-dialog :title="title" :visible="visible" :before-close="beforeClose"></kui-dialog>
        </div>
      `,
  ...
  it('destroyOnClose', async() => {
    vm = createVue({
      template: `
        <div>
          <kui-dialog :title="title" :visible.sync="visible" destroy-on-close>
            <input />
          </kui-dialog>
        </div>
      `,
