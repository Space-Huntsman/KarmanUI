...
describe('Select', () => {
  ...
  it('create', () => {
    ...
    expect(vm.$el.className).to.equal('kui-select');
    expect(vm.$el.querySelector('.kui-input__inner').placeholder).to.equal('请选择');
    vm.toggleMenu();
  ...
  it('options rendered correctly', () => {
    vm = getSelectVm();
    const options = vm.$el.querySelectorAll('.kui-select-dropdown__item');
  ...
  it('custom dropdown class', () => {
    ...
    const dropdown = vm.$el.querySelector('.kui-select-dropdown');
  ...
  it('default value', done => {
    ...
    setTimeout(() => {
      expect(vm.$el.querySelector('.kui-input__inner').value).to.equal('双皮奶');
      done();
    }, 100);
  });
  ...
  it('single select', done => {
    vm = createVue({
      template: `
        <div>
          <kui-select v-model="value" @change="handleChange">
            <kui-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('disabled select', () => {
    ...
    expect(vm.$el.querySelector('.kui-input').classList.contains('is-disabled')).to.true;
  });
  ...
  it('visible event', done => {
    vm = createVue({
      template: `
        <div>
          <kui-select v-model="value" @visible-change="handleVisibleChange">
            <kui-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('clearable', done => {
    ...
    setTimeout(() => {
      const iconClear = vm.$el.querySelector('.kui-input__icon.kui-icon-circle-close');
  ...
  it('object typed value', done => {
    vm = createVue({
      template: `
        <div>
          <kui-select v-model="value" value-key="id">
            <kui-option
              v-for="item in options"
              :label="item.label"
              :key="item.id"
    ...
    setTimeout(() => {
      expect(vm.$el.querySelector('.kui-input__inner').value).to.equal('label1');
      expect(vm.$el.querySelector('.kui-select-dropdown__item').classList.contains('selected'));
      done();
    }, 100);
  });
  ...
  it('prefixed icon', () => {
      ...
      template: `
        <div>
          <kui-select v-model="value">
            ...
            <i slot="prefix" class="kui-input__icon kui-icon-search"></i>
          </kui-select>
        </div>
      `,
    ...
    expect(vm.$el.querySelector('.kui-input__icon').classList.contains('kui-icon-search')).to.be.true;
  });

  it('custom kui-option template', () => {
    vm = createVue({
      template: `
        <div>
          <kui-select v-model="value">
    ...
    expect(vm.$el.querySelector('.kui-select-dropdown__item p').textContent).to.equal('label value');
  });
  ...
  it('option group', () => {
    ...
    const groups = vm.$el.querySelectorAll('.kui-select-group__wrap');
    const options = groups[1].querySelectorAll('.kui-select-dropdown__item');
  ...
  it('allow create', done => {
    ...
    setTimeout(() => {
      ...
      select.onInputChange();
      setTimeout(() => {
        const options = document.querySelectorAll('.kui-select-dropdown__item span');
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          const tagCloseIcons = vm.$el.querySelectorAll('.kui-tag__close');
  ...
  it('multiple remove-tag', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <kui-select v-model="value" multiple @remove-tag="handleRemoveTag">
            <kui-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('event:focus & blur', done => {
    vm = createVue({
      template: `
        <kui-select ref="select"></kui-select>
      `
    }, true);
  ...
  it('should return focus to input inside select after option select', done => {
    vm = createVue({
      template: `
        <div>
          <kui-select v-model="value" ref="select">
            <kui-option label="1" :value="1" />
          </kui-select>
        </div>
      `,
      data() {
    ...
    const option = vm.$el.querySelectorAll('.kui-select-dropdown__item')[0];
  ...
  it('only emit change on user input', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <kui-select v-model="value" @change="change" ref="select">
            <kui-option label="1" :value="1" />
            <kui-option label="2" :value="2" />
            <kui-option label="3" :value="3" />
          </kui-select>
        </div>
      `,
      data() {
  ...
  it('should set placeholder to label of selected option when filterable is true and multiple is false', async() => {
    vm = createVue({
      template: `
        <div>
          <kui-select ref="select" v-model="value" filterable>
            <kui-option label="test" value="test" />
          </kui-select>
        </div>
      `,
      data() {
    ...
    expect(vm.$el.querySelector('.kui-input__inner').placeholder).to.be.equal('test');
  ...
  it('default value is null or undefined', async() => {
    ...
    vm.value = null;
    await waitImmediate();
    expect(vm.$el.querySelector('.kui-input__inner').value).to.equal('');
    vm.value = '选项1';
    await waitImmediate();
    expect(vm.$el.querySelector('.kui-input__inner').value).to.equal('黄金糕');
  });
