...
describe('Loading', () => {
  ...
  describe('as a directive', () => {
    it('create', done => {
      ...
      Vue.nextTick(() => {
        const mask = vm.$el.querySelector('.kui-loading-mask');
    ...
    it('unbind', done => {
      ...
      Vue.nextTick(() => {
        ...
        Vue.nextTick(() => {
          ...
          vm2.show = false;
          Vue.nextTick(() => {
            expect(document.querySelector('.kui-loading-mask')).to.not.exist;
            done();
          });
        });
      });
    ...
    it('body', done => {
      ...
      Vue.nextTick(() => {
        const mask = document.querySelector('.kui-loading-mask');
    ...
    it('lock', done => {
      ...
      Vue.nextTick(() => {
        ...
        vm.loading = false;
        document.body.removeChild(document.querySelector('.kui-loading-mask'));
        document.body.removeChild(vm.$el);
        done();
      });
    });
    ...
    it('text', done => {
      vm = createVue({
        template: `
        <div v-loading="loading" karman-loading-text="拼命加载中"></div>
      `,
      ...
      Vue.nextTick(() => {
        const mask = document.querySelector('.kui-loading-mask');
        const text = mask.querySelector('.kui-loading-text');
    ...
    it('customClass', done => {
      vm = createVue({
        template: `
        <div v-loading="loading" karman-loading-custom-class="loading-custom-class"></div>
      `,
  ...
  describe('as a service', () => {
    it('create', () => {
      ...
      expect(document.querySelector('.kui-loading-mask')).to.exist;
    });
    ...
    it('target', done => {
      ...
      let mask = document.querySelector('.kui-loading-mask');
    ...
    it('fullscreen singleton', done => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          let masks = document.querySelectorAll('.kui-loading-mask');
          expect(masks.length).to.equal(1);
          loadingInstance2.close();
          setTimeout(() => {
            masks = document.querySelectorAll('.kui-loading-mask');
    ...
    it('text', () => {
      ...
      const text = document.querySelector('.kui-loading-text');
      expect(text).to.exist;
      expect(text.textContent).to.equal('Loading...');
    });

    it('customClass', () => {
      loadingInstance = Loading({ customClass: 'kui-loading-custom-class' });
      const customClass = document.querySelector('.kui-loading-custom-class');
