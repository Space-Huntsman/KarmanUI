...
describe('Form', () => {
  ...
  it('label width', done => {
    ...
    expect(vm.$el.querySelector('.kui-form-item__label').style.width).to.equal('80px');
    expect(vm.$el.querySelector('.kui-form-item__content').style.marginLeft).to.equal('80px');
    done();
  });
  it('auto label width', async() => {
    vm = createVue({
      template: `
        <kui-form ref="form" :model="form" labkui-width="auto">
          ...
          <kui-form-item label="活动备注信息" v-if="display">
            <kui-input v-model="form.name"></kui-input>
          </kui-form-item>
        </kui-form>
      `,
    ...
    const formItems = vm.$el.querySelectorAll('.kui-form-item__content');
    ...
    const formItem = vm.$el.querySelector('.kui-form-item__content');
  ...
  it('inline form', done => {
    vm = createVue({
      template: `
        <kui-form ref="form" :model="form" inline>
          <kui-form-item>
            <kui-input v-model="form.name"></kui-input>
          </kui-form-item>
          <kui-form-item>
    ...
    expect(vm.$el.classList.contains('kui-form--inline')).to.be.true;
    done();
  });
  ...
  it('label size', () => {
    vm = createVue({
      template: `
        <div>
          <kui-form :model="form" size="mini" ref="labelMini">
            <kui-form-item>
              <kui-input v-model="form.name"></kui-input>
            </kui-form-item>
          </kui-form>
    ...
    expect(vm.$refs.labelMini.$el.children[0].classList.contains('kui-form-item--mini')).to.be.true;
  });
  it('show message', done => {
    vm = createVue({
      template: `
        <kui-form :model="form" ref="form">
          <kui-form-item label="活动名称" prop="name" :show-message="false"
            :rules="{
              required: true,
              message: '请输入活动名称',
              trigger: 'change',
    ...
    vm.$refs.form.validate(valid => {
      ...
      vm.$refs.form.$nextTick(_ => {
        expect(vm.$el.querySelector('.kui-form-item__error')).to.not.exist;
        done();
      });
    });
  });
  it('reset field', done => {
    vm = createVue({
      template: `
        <kui-form ref="form" :model="form" :rules="rules">
          <kui-form-item label="活动名称" prop="name">
            <kui-input v-model="form.name" ref="fieldName"></kui-input>
          </kui-form-item>
          <kui-form-item label="活动地址" prop="address">
            <kui-input v-model="form.address" ref="fieldAddr"></kui-input>
          </kui-form-item>
  ...
  describe('validate', () => {
    it('input', done => {
      vm = createVue({
        template: `
          <kui-form :model="form" :rules="rules" ref="form">
            <kui-form-item label="活动名称" prop="name" ref="field">
              <kui-input v-model="form.name"></kui-input>
            </kui-form-item>
          </kui-form>
        `,
    ...
    it('textarea', done => {
      vm = createVue({
        template: `
          <kui-form :model="form" :rules="rules" ref="form">
            <kui-form-item label="活动名称" prop="name" ref="field">
              <kui-input type="textarea" v-model="form.name"></kui-input>
            </kui-form-item>
          </kui-form>
        `,
        data() {
    ...
    it('selector', done => {
      vm = createVue({
        template: `
          <kui-form :model="form" :rules="rules" ref="form">
            <kui-form-item label="记住密码" prop="region" ref="field">
              <kui-select v-model="form.region" placeholder="请选择活动区域">
                <kui-option label="区域一" value="shanghai"></kui-option>
                <kui-option label="区域二" ref="opt" value="beijing"></kui-option>
              </kui-select>
            </kui-form-item>
          </kui-form>
        `,
    ...
    it('datepicker', done => {
      vm = createVue({
        template: `
          <kui-form :model="form" :rules="rules" ref="form">
            <kui-form-item label="记住密码" prop="date" ref="field">
              <kui-date-picker type="date" ref="picker" placeholder="选择日期" v-model="form.date" style="width: 100%;"></kui-date-picker>
            </kui-form-item>
          </kui-form>
        `,
        data() {
    ...
    it('timepicker', done => {
      vm = createVue({
        template: `
          <kui-form :model="form" :rules="rules" ref="form">
            <kui-form-item label="记住密码" prop="date" ref="field">
              <kui-time-picker ref="picker" placeholder="选择时间" v-model="form.date" style="width: 100%;"></kui-time-picker>
            </kui-form-item>
          </kui-form>
        `,
        data() {
    ...
    it('checkbox', done => {
      vm = createVue({
        template: `
          <kui-form :model="form" :rules="rules" ref="form">
            <kui-form-item label="是否接受协议" prop="accept" ref="field">
              <kui-checkbox v-model="form.accept">
                <span>接受协议</span>
              </kui-checkbox>
            </kui-form-item>
          </kui-form>
    ...
    it('checkbox group', done => {
      vm = createVue({
        template: `
          <kui-form :model="form" :rules="rules" ref="form">
            <kui-form-item label="活动名称" prop="type" ref="field">
              <kui-checkbox-group v-model="form.type">
                <kui-checkbox label="美食/餐厅线上活动" name="type"></kui-checkbox>
                <kui-checkbox label="地推活动" name="type"></kui-checkbox>
                <kui-checkbox label="线下主题活动" name="type"></kui-checkbox>
    ...
    it('radio group', done => {
      vm = createVue({
        template: `
          <kui-form :model="form" :rules="rules" ref="form">
            <kui-form-item label="活动名称" prop="type" ref="field">
              <kui-radio-group v-model="form.type">
    ...
    it('error', done => {
      vm = createVue({
        template: `
          <kui-form :model="form" :rules="rules" ref="form">
            <kui-form-item label="活动名称" prop="name" :error="error" ref="field">
              <kui-input v-model="form.name"></kui-input>
            </kui-form-item>
          </kui-form>
        `,
  ...
  it('validate event', done => {
    vm = createVue({
      template: `
          <kui-form :model="form" :rules="rules" ref="form" @validate="onValidate">
            <kui-form-item label="活动名称" prop="name" ref="name">
              <kui-input v-model="form.name"></kui-input>
            </kui-form-item>
            <kui-form-item label="活动地点" prop="addr" ref="addr">
              <kui-input v-model="form.addr"></kui-input>
            </kui-form-item>
          </kui-form>
        `,
      data() {
