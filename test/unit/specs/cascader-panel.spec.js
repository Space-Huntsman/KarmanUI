...
const getValidOptions = (el, menuIndex) => getMenus(el)[menuIndex].querySelectorAll('.kui-cascader-node[tabindex="-1"]');
const getLabel = el => el.querySelector('.kui-cascader-node__label').textContent;
...
describe('CascaderPanel', () => {
    ...
    vm = createVue({
      template: `
        <kui-cascader-panel
          ref="panel"
          v-model="value"
          :options="options"></kui-cascader-panel>
      `,
      data() {
        return {
          value: [],
  ...
  it('with default value', async() => {
    ...
    expect(getOptions(el, 2)[0].querySelector('.kui-icon-check')).to.exist;
  });
  ...
  it('expand by hover', async() => {
    vm = createVue({
      template: `
        <kui-cascader-panel
          :options="options"
          :props="props"></kui-cascader-panel>
      `,
      data() {
        return {
          options,
  ...
  it('multiple mode', async() => {
    ...
    const el = vm.$el;
    const checkbox = getOptions(el, 0)[0].querySelector('.kui-checkbox');
    expect(checkbox).to.exist;
    expect(checkbox.querySelector('.kui-checkbox__input').className).to.not.includes('is-checked');
    checkbox.querySelector('input').click();

    await waitImmediate();
    expect(checkbox.querySelector('.kui-checkbox__input').className).to.includes('is-checked');
  ...
  it('multiple mode with disabled default value', async() => {
    ...
    await waitImmediate();
    expect(checkbox).to.exist;
    expect(checkbox.querySelector('.kui-checkbox__input').className).to.includes('is-indeterminate');
    checkbox.querySelector('input').click();
  ...
  it('check strictly in single mode', async() => {
    ...
    const el = vm.$el;
    const radio = getOptions(el, 0)[0].querySelector('.kui-radio');
    ...
    expect(getOptions(el, 0)[1].querySelector('.kui-radio').className).to.includes('is-disabled');
  });
  ...
  it('check strictly in multiple mode', async() => {
    ...
    getOptions(el, 0)[0].click();
    await waitImmediate();
    expect(getOptions(el, 1)[0].querySelector('.kui-checkbox').className).to.not.includes('is-checked');
    getOptions(el, 1)[0].querySelector('input').click();
    await waitImmediate();
    expect(vm.value).to.deep.equal(['zhejiang', 'hangzhou']);
    expect(getOptions(el, 0)[1].querySelector('.kui-checkbox').className).to.includes('is-disabled');
  });
  ...
  it('dynamic loading', async() => {
    ...
    firstOption.click();
    await waitImmediate();
    expect(firstOption.querySelector('i').className).to.includes('kui-icon-loading');
    await wait(1000);
    expect(firstOption.querySelector('i').className).to.includes('kui-icon-arrow-right');
    expect(getMenus(el).length).to.equal(2);
    getOptions(el, 1)[0].click();
    await wait(1000);
    getOptions(el, 2)[0].click();
