...
:::demo Après avoir configuré l'attribut `data` de `kui-table` avec un tableau d'objets, vous pouvez utiliser `prop` (correspondant à une clé dans chaque objet du tableau `data`) dans `kui-table-column` pour insérer des données dans les colonnes, ainsi que l'attribut `label` pour définir le nom de la colonne. L'attribut `width` définit si besoin la largeur de la colonne.
...
:::demo Utilisez `row-class-name` dans `kui-table` pour utiliser une fonction qui permettra ajouter des classes à certaines lignes. Vous pouvez ensuite définir les classes en question dans votre CSS.
```html
<template>
  <kui-table
    :data="tableData"
...
:::demo En réglant l'attribut `height` de `kui-table`, vous pouvez fixer le header sans avoir besoin de plus de code.
```html
<template>
  <kui-table
    :data="tableData"
...
:::demo Utilisez l'attribut `fixed` de `kui-table-column` qui accepte un `Boolean`. Si `true`, la colonne sera fixée à gauche. Il accepte aussi les valeurs 'left' et 'right', indiquant de quel coté la colonne doit être fixée.
```html
<template>
  <kui-table
    :data="tableData"
    ...
    <kui-table-column
      ...
      <template slot-scope="scope">
        ...
        <kui-button type="text" size="small">Editer</kui-button>
      </template>
    </kui-table-column>
  </kui-table>
</template>
...
:::demo En utilisant l'attribut `max-height` de `kui-table`, vous fixez le header. Le tableau ne défilera que si la hauteur des lignes dépasse cette hauteur maximale.
```html
<template>
  <kui-table
    :data="tableData"
...
:::demo Placez simplement `kui-table-column` dans une autre `kui-table-column`, et vous grouperez ainsi les headers.
```html
<template>
  <kui-table
    :data="tableData"
    ...
    <kui-table-column label="Informations de livraison">
      <kui-table-column
        prop="name"
        label="Nom"
        width="120">
      </kui-table-column>
      <kui-table-column label="Informations d'adresse">
        <kui-table-column
          prop="state"
          label="État"
          width="120">
...
<template>
  ...
  <div style="margin-top: 20px">
    <kui-button @click="setCurrent(tableData[1])">Sélectionner la deuxième ligne</kui-button>
    <kui-button @click="setCurrent()">Effacer la sélection</kui-button>
  </div>
</template>
...
<template>
  ...
  <div style="margin-top: 20px">
    <kui-button @click="toggleSelection([tableData[1], tableData[2]])">Sélectionner les deuxième et troisième lignes</kui-button>
    <kui-button @click="toggleSelection()">Effacer la sélection</kui-button>
  </div>
</template>
...
:::demo Réglez `filters` et `filter-method` dans `kui-table-column` pour rendre la colonne filtrable. `filters` prends un tableau, et `filter-method` est une fonction déterminant comment les lignes s'affichent. Elle prend trois paramètres: `value`, `row` et `column`.
```html
<template>
  <kui-button @click="resetDateFilter">Effacer le filtre date</kui-button>
  <kui-button @click="clearFilter">Effacer tout les filtres</kui-button>
  <kui-table
    ref="filterTable"
    :data="tableData"
    style="width: 100%">
  ...
  <kui-table
    ...
    <kui-table-column
      ...
      <template slot-scope="scope">
        <kui-button
          size="mini"
          @click="handleEdit(scope.$index, scope.row)">Editer</kui-button>
        <kui-button
          size="mini"
          type="danger"
          @click="handleDelete(scope.$index, scope.row)">Supprimer</kui-button>
      </template>
    </kui-table-column>
  </kui-table>
</template>
...
:::demo Activer la ligne extensible en mettant `type` à `expand` et en utilisant un slot. Le contenu de `kui-table-column` sera généré quand l'extension aura lieu, et vous avez aux même attributs que pour le slot des templates de colonnes.
```html
<template>
  <kui-table
    :data="tableData"
...
:::demo Utilisez l'attribut `index` sur une `kui-table-column` avec `type=index`. Si un nombre est assigné, tout les indices auront un décalage égal à ce nombre. Il peut aussi prendre une fonction avec chaque indice (commençant à `0`) comme paramètre, et la valeur de retour sera affichée en tant qu'indice.
...
| resizable | Si la largeur de la colonne peut être modifiée, marche quand `border` de `kui-table` est `true`. | boolean | — | false |
...
| labkui-class-name | Classe du label de cette colonne. | string | — | — |
