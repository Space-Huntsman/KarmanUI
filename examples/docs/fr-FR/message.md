...
```html
<template>
  <kui-button :plain="true" @click="openCenter">Texte centré</kui-button>
</template>
...
```html
<template>
  <kui-button :plain="true" @click="openHTML">Utiliser du HTML</kui-button>
</template>
...
Karman ajoute une méthode `$message` à Vue.prototype. Vous pouvez donc appeler `Message` dans l'instance de Vue comme dans cette page.
