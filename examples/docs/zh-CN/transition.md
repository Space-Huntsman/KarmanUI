...
Karman 内应用在部分组件的过渡动画，你也可以直接使用。在使用之前请阅读 [transition 组件文档](https://cn.vuejs.org/v2/api/#transition) 。

### fade 淡入淡出

:::demo 提供 `kui-fade-in-linear` 和 `kui-fade-in` 两种效果。
```html
<template>
  <div>
...
:::demo 提供 `kui-zoom-in-center`，`kui-zoom-in-top` 和 `kui-zoom-in-bottom` 三种效果。
```html
<template>
  <div>
...
使用 `kui-collapse-transition` 组件实现折叠展开效果。
