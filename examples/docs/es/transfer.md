...
### Uso básico
:::demo Los datos se pasan a Transfer a través del atributo `data`. Los datos tienen que ser un array de objetos, y cada objeto debe tener estos atributos: `key` que será el identificador del ítem, `label` que será el texto a mostrar, y `disabled` que indicará si el karmano esta desactivado. Los items dentro de la lista destino están sincronizados con la variable asociada a `v-model`, y el valor de esa variable es un array de claves de los karmanos de la lista destino. Así que si no quiere que la lista destino esté vacía inicialmente puede inicializar el `v-model` con un array.
```html
<template>
  <kui-transfer
    v-model="value"
...
Por defecto Transfer busca los atributos `key`, `label`, y `disabled` en cada karmano. Si sus datos tienen un nombre diferente para la clave puede usar el atributo `props` para añadir alias.

:::demo En este ejemplo los karmanos no tienen `key`y `label`, en vez de eso tienen `value` y `desc`. Así que tiene que añadir alias para `key` y `label`.
```html
<template>
  <kui-transfer
    v-model="value"
...
| target-order          | estrategia de órdenes para karmanos de la lista destino. Si está configurado en `original`, los karmanos mantendrán el mismo orden que la fuente de datos. Si está configurado para `push`, los nuevos karmanos añadidos se insertaran al final. Si se ajusta a `unshift`, los nuevos karmanos añadidos se insertarán en la parte superior. | string | original / push / unshift | original |
...
| left-default-checked  | Array de claves de los karmanos marcados inicialmente en la lista de la izquierda | array                           | —                 | [ ]                                      |
| right-default-checked | Array de claves de los karmanos marcados inicialmente en la lista de la derecha | array                           | —                 | [ ]                                      |
...
| ------ | ---------------------------------------- | ---------------------------------------- |
| change | se lanza cuando los karmanos cambian en la lista de la derecha | array con las claves de los karmanos de la lista de la derecha |
| left-check-change | se dispara cuando el usuario final cambia el estado verificado de cualquier karmano de datos en la lista izquierda | array clave de ítems actualmente verificados, array clave de ítems cuyo estado verificado ha cambiado |
| right-check-change | se dispara cuando el usuario final cambia el estado verificado de cualquier karmano de datos en la lista derecha. | array clave de ítems actualmente verificados, array clave de ítems cuyo estado verificado ha cambiado |
