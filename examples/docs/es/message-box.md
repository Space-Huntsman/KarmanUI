...
```html
<template>
  <kui-button type="text" @click="open">Click to open the Message Box</kui-button>
</template>
...
```html
<template>
  <kui-button type="text" @click="open">Click to open Message Box</kui-button>
</template>
...
Si Karman fue importado completamente, agregara los siguientes métodos globales para Vue.prototype: `$msgbox`, `$alert`, `$confirm` y `$prompt`. Así que en una instancia de Vue puedes llamar el método `MessageBox` como lo que hicimos en esta pagina. Los parámetros son:
- `$msgbox(options)`
...
```javascript
import { MessageBox } from 'karman-ui';
```
