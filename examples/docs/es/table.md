...
:::demo Después de haber establecido el atributo `data` de `kui-table` con un arreglo de objetos, puede usar la propiedad `prop` (el correspondiente a la clave de un objeto dentro del arreglo `data`) en `kui-table-column` para insertar datos a las columnas de la tabla, y establecer el atributo `label` para definir el nombre de la columna. También puede usar el atributo `width` para establecer el ancho de las columnas.
...
:::demo Utilice `row-class-name` en `kui-table` para agregar clases personalizadas a una fila en específico. Y entonces, podrá darle diseño con estas clases.
```html
<template>
  <kui-table
    :data="tableData"
...
:::demo Al configurar el atributo `height` de `kui-table`, puede fijar la cabecera de la tabla sin agregar otro código.
```html
<template>
  <kui-table
    :data="tableData"
...
:::demo El atributo `fixed` es utilizado en `kui-table-column`, este acepta un `Boolean`. Si es `true`, la columna será fijada a la izquierda. También acepta dos tipos: `left` y `right`, ambos indican donde debe ser fijada la columna.
```html
<template>
  <kui-table
    :data="tableData"
    ...
    <kui-table-column
      ...
      <template slot-scope="scope">
        <kui-button @click="handleClick" type="text" size="small">Detalle</kui-button>
        <kui-button type="text" size="small">Editar</kui-button>
      </template>
    </kui-table-column>
  </kui-table>
</template>
...
:::demo Al configurar el atributo `max-height` de `kui-table`, tu puedes fijar la cabecera de la tabla. La barra de desplazamiento únicamente se mostrará si la altura sobrepasa el valor de la altura máxima.
```html
<template>
  <kui-table
    :data="tableData"
...
:::demo Solo necesitas colocar `kui-table-column` dentro de un `kui-table-column`, de esta forma lograrás agruparlas.
```html
<template>
  <kui-table
    :data="tableData"
    ...
    <kui-table-column label="Información de entrega">
      <kui-table-column
        prop="name"
        label="Nombre"
        width="120">
      </kui-table-column>
      <kui-table-column label="Información de dirección">
        <kui-table-column
          prop="state"
          label="Estado"
          width="120">
...
<template>
  ...
  <div style="margin-top: 20px">
    <kui-button @click="setCurrent(tableData[1])">Seleccionar segunda fila</kui-button>
    <kui-button @click="setCurrent()">Limpiar selección</kui-button>
  </div>
</template>
...
<template>
  ...
  <div style="margin-top: 20px">
    <kui-button @click="toggleSelection([tableData[1], tableData[2]])">Cambia el estado de selección de la segunda y tercera fila.</kui-button>
    <kui-button @click="toggleSelection()">Limpiar selección</kui-button>
  </div>
</template>
...
:::demo Establezca el atributo `filters` y `filter-method` en `kui-table-column` haciendo esta columna filtrable. `filters` es un arreglo, y `filter-method` es una función que decide que filas se muestra. Esta tiene tres parámetros: `value`, `row` y `column`.
```html
<template>
  ...
  <kui-table
    ...
    <kui-table-column
      ...
      <template slot-scope="scope">
        <kui-button
          size="mini"
          @click="handleEdit(scope.$index, scope.row)">Editar</kui-button>
        <kui-button
          size="mini"
          type="danger"
          @click="handleDelete(scope.$index, scope.row)">Eliminar</kui-button>
      </template>
    </kui-table-column>
  </kui-table>
</template>
...
:::demo Puede activar la fila expandible estableciendo la propiedad `type` a `expand` y Scoped Slots. La plantilla para `kui-table-column` se representará como el contenido de la fila expandible, y puede acceder a algunos atributos cuando está usando `Scoped Slots` en plantillas de columna personalizadas.
```html
<template>
  <kui-table
    :data="tableData"
...
| sort-orders           | el orden de las estrategias de ordenación utilizadas al ordenar los datos, funciona cuando `sortable` es `true`. Acepta un array, a medida que el usuario hace clic en el encabezado, la columna se ordena en el orden de los karmanos del array. | array | los karmanos en el arreglo necesitan ser uno de los siguientes: `ascending`, `descending` y `null` (restaura el orden original) | ['ascending', 'descending', null] |
| resizable             | especifica si el ancho de la columna puede ser redimensionado, funciona cuando `border` de `kui-table` está en `true` | boolean                           | —                             | false       |
...
| labkui-class-name      | nombre de clase de la etiqueta de esta columna | string                            | —                             | —           |
...
| filters               | un arreglo de opciones para filtrado de datos. Para cada karmano en este arreglo, `text` y `value` son obligatorios | Array[{ text, value }]            | —                             | —           |
