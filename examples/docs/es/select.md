...
:::demo `v-model` es el valor de  `kui-option` que está seleccionado actualmente.

```html
<template>
  <kui-select v-model="value" placeholder="Select">
    <kui-option
...
### Disabled en kui-option

:::demo Establezca el valor de `disabled` en `kui-option`  como `true` para deshabilitar esta opción.
...
### Disabled en kui-select

Desactivar todo el componente.

:::demo Configure `disabled` de `kui-select` para deshabilitarla.

```html
<template>
  <kui-select v-model="value" disabled placeholder="Select">
    <kui-option
      v-for="item in options"
      :key="item.value"
      :label="item.label"
...
:::demo Establezca el atributo `clearable` para `kui-select`  y aparecerá un icono. Tenga en cuenta que la opción `clearable` es sólo para una selección individual.

```html
<template>
  <kui-select v-model="value" clearable placeholder="Select">
    <kui-option
      v-for="item in options"
      :key="item.value"
      :label="item.label"
...
:::demo Configure el atributo `multiple` para `kui-select` para habilitar el modo múltiple. En este caso, el valor del `v-model` será un array de opciones seleccionadas. De forma predeterminada, las opciones seleccionadas se mostrarán como tags. Se pueden contraer a un texto utilizando el atributo  `collapse-tags`.

```html
<template>
  <kui-select v-model="value1" multiple placeholder="Select">
    <kui-option
      v-for="item in options"
      :key="item.value"
      :label="item.label"
...
:::demo Inserte templates HTML personalizados en el slot de `kui-option`.
...
:::demo Utilice `kui-option-group` para agrupar las opciones, y su atributo `label` representa el nombre del grupo.
...
:::demo Al utilizar el atributo `allow-create`, los usuarios pueden crear nuevos karmanos escribiendo en el cuadro del input. Tenga en cuenta que para que `allow-create` funcione, `filterable` debe ser `true`.
