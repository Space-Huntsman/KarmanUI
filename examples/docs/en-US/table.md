...
:::demo After setting attribute `data` of `kui-table` with an object array, you can use `prop` (corresponding to a key of the object in `data` array) in `kui-table-column` to insert data to table columns, and set the attribute `label` to define the column name. You can also use the attribute `width` to define the width of columns.
...
:::demo Use `row-class-name` in `kui-table` to add custom classes to a certain row. Then you can style it with custom classes.
```html
<template>
  <kui-table
    :data="tableData"
...
:::demo By setting the attribute `height` of `kui-table`, you can fix the table header without any other codes.
```html
<template>
  <kui-table
    :data="tableData"
...
:::demo Attribute `fixed` is used in `kui-table-column`, it accepts a `Boolean`. If `true`, the column will be fixed at left. It also accepts two string literals: 'left' and 'right', both indicating that the column will be fixed at corresponding direction.
```html
<template>
  <kui-table
    :data="tableData"
    ...
    <kui-table-column
      ...
      <template slot-scope="scope">
        <kui-button @click="handleClick" type="text" size="small">Detail</kui-button>
        <kui-button type="text" size="small">Edit</kui-button>
      </template>
    </kui-table-column>
  </kui-table>
</template>
...
:::demo  By setting the attribute `max-height` of `kui-table`, you can fix the table header. The table body scrolls only if the height of the rows exceeds the max height value.
```html
<template>
  <kui-table
    :data="tableData"
...
:::demo Only need to place kui-table-column inside a kui-table-column, you can achieve group header.
```html
<template>
  <kui-table
    :data="tableData"
    ...
    <kui-table-column label="Delivery Info">
      <kui-table-column
        prop="name"
        label="Name"
        width="120">
      </kui-table-column>
      <kui-table-column label="Address Info">
        <kui-table-column
          prop="state"
          label="State"
          width="120">
...
<template>
  ...
  <div style="margin-top: 20px">
    <kui-button @click="setCurrent(tableData[1])">Select second row</kui-button>
    <kui-button @click="setCurrent()">Clear selection</kui-button>
  </div>
</template>
...
<template>
  ...
  <div style="margin-top: 20px">
    <kui-button @click="toggleSelection([tableData[1], tableData[2]])">Toggle selection status of second and third rows</kui-button>
    <kui-button @click="toggleSelection()">Clear selection</kui-button>
  </div>
</template>
...
:::demo Set attribute `filters` and `filter-method` in `kui-table-column` makes this column filterable. `filters` is an array, and `filter-method` is a function deciding which rows are displayed. It has three parameters: `value`, `row` and `column`.
```html
<template>
  <kui-button @click="resetDateFilter">reset date filter</kui-button>
  <kui-button @click="clearFilter">reset all filters</kui-button>
  <kui-table
    ref="filterTable"
    :data="tableData"
    style="width: 100%">
...
:::demo Activate expandable row by adding type="expand" and scoped slot. The template for kui-table-column will be rendered as the contents of the expanded row, and you can access the same attributes as when you are using `Scoped slot` in custom column templates.
```html
<template>
  <kui-table
    :data="tableData"
...
:::demo To customize row indices, use `index` attribute on `kui-table-column` with `type=index`. If it is assigned to a number, all indices will have an offset of that number. It also accepts a method with each index (starting from `0`) as parameter, and the returned value will be displayed as index.
...
| height | Table's height. By default it has an `auto` height. If its value is a number, the height is measured in pixels; if its value is a string, the value will be assigned to karman's style.height, the height is affected by external styles | string/number | — | — |
...
| resizable | whether column width can be resized, works when `border` of `kui-table` is `true` | boolean | — | false |
...
| labkui-class-name | class name of the label of this column | string | — | — |
...
| filters | an array of data filtering options. For each karman in this array, `text` and `value` are required | Array[{ text, value }] | — | — |
