<template>
  <div class="ext-panel" :class="{moving : moving}" :style="{top: `${top}px`, left: `${left}px`}" ref="editor" >
    ...
    <div class="editor" :style="{height: `${height}px`}" v-show="showSidebar">
      <kui-tabs v-model="activeTab" @tab-click="onTabClick">
        <kui-tab-pane label="Config" name="config">
          <theme-configurator
        ...
        <kui-tab-pane label="Gallery" name="gallery">
          <gallery
            ref='gallery'
            :height="height"
            :width="width - 7"
...
export default {
  ...
  mounted() {
    ...
    // chrome.storage.local.remove('KUI_THEME_USER_CONFIG');
...
<style>
.ext-panel  .editor .kui-tabs__content, .ext-panel  .editor .kui-tabs--top, .ext-panel  .editor .kui-tab-pane {
  height: 100%;
}
.ext-panel .kui-tabs__nav-scroll >div {
