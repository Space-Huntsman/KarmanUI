...
<template>
  <div class="page-container page-theme">
    ...
    <kui-dialog :visible.sync="copyDialogVisible">
      <kui-form :model="copyForm" ref="copyForm" :rules="copyFormRule">
        <kui-form-item label="<%= 3 >" prop="name">
          <kui-input v-model="copyForm.name"></kui-input>
        </kui-form-item>
      </kui-form>
      <div slot="footer" class="dialog-footer">
        <kui-button @click="closeCopyForm">{{getActionDisplayName('cancel')}}</kui-button>
        <kui-button type="primary" @click="copyToUser">{{getActionDisplayName('confirm')}}</kui-button>
      </div>
    </kui-dialog>
  </div>
</template>
