...
<template>
  <div class="component-preview">
    ...
    <div class="color">
      <kui-row :gutter="12">
        <kui-col :span="4" v-for="(color, key) in colorLine" :key="key">
...
export default {
  created() {
    ...
    bus.$on(ACTION_COMPONECT_SELECT, (val) => {
      this.$nextTick(() => {
        const getSelectKarman = Array.from(document.querySelectorAll('h4')).filter((el) => (el.innerText.toLowerCase() === val));
        if (getSelectKarman[0]) {
          const karmanTop = getSelectKarman[0].getBoundingClientRect().top;
          window.scrollTo(0, window.pageYOffset + karmanTop - 20); // 20 for padding
        }
      });
    });
  },
