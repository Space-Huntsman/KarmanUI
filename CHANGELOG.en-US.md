...
- Transfer
  - Fix incorrect line-height of kui-transfer's first list item when it was used with  kui-form-item (#18917 by @Hanx)
- InputNumber
...
- Calendar
  - Import kui-button and kui-button-group (#17376 by @masongzhi)
- MessageBox
...
- InfiniteScroll
  - Skip trigger event on invisible karman (#17553 by @iamkun)
- Image
...
- Theme
  - Add Chrome Extension: Karman Theme Extension (#16686 by @iamkun)
- Icon
...
- Tree
  - Make kui-tree generic (#15934 by @JeremyWuuuuu)
...
- Input
  - Fix show-word-count style in kui-form (#15359 by @lvjiaxuan)
...
- Switch
  - Label with kui-switch repeating event (#15178 by @FAKER-A)
- Slider
...
- Form
  - Fix switching the rules in kui-form not work (#14985 by @luckyCao)
...
- Form
  - Fix auto `labkui-width` style (#14955 by @ziyoung)
...
- Form
  - `labkui-width` supports auto width (#14944 by @ziyoung)
...
- NavMenu
  - Fix click kui-submenu trigger childMenu pop again bug (#14443 by @PanJiaChen)
- Dropdown
...
- DropdownItem
  - Add icon prop to `kui-dropdown-item` (by @gabrielboliveira in #14088)
- Input
...
- Menu
  - Add support for `kui-menu-item` without index (by @georgyfarniev in #13298)
- Table
...
- Build
  - Minimizing css and js for Karman doc site (by @iamkun in #14430)
...
- Now `class-name` and `labkui-class-name` of Table are reactive, #11626
...
- Popover
  - Fixed the console error when the triggering karman is Radio or Checkbox, #11265
- Breadcrumb
...
- MessageBox
  - Fixed the `kui-popup-parent--hidden` class not removed when opening MessageBox in succession, #11371
...
- The `lockScroll` attribute of pop up components no longer adds an inline style to the parent karman, but instead adds a class name, #11114
...
- Now we use ResizeObserver to detect DOM karman resizing, #10779
...
- Fixed a regression that `type` of Input is not passed down to the native input karman, #10415
...
- Now Loading directive supports custom class name by assigning `karman-loading-custom-class` attribute, #8826 (by @earlymeme)
...
- General
  ...
  - Now you can configure component sizes globally. When importing Karman, you can add a global config object with a `size` prop to configure default sizes for all components.
- Button
...
- Table
  - Fixed a bug that Table remains hiding when its parent karman appears from `display: none`
  - Fixed Table expanding its width when its parent karman has `display: flex`
...
- Input
  ...
  - `change` event now behaves like the native input karman, which triggers only on blur or pressing enter. If you need to respond to user input in real time, you can use `input` event.
- Autocomplete
...
- Table
  ...
  - `append` slot is moved outside the `tbody` karman to avoid multiple rendering
