...
export const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map(week => t(`long.datepicker.weeks.${ week }`)),
    dayNames: weeks.map(week => t(`long.datepicker.weeks.${ week }`)),
    monthNamesShort: months.map(month => t(`long.datepicker.months.${ month }`)),
    monthNames: months.map((month, index) => t(`long.datepicker.month${ index + 1 }`)),
    amPm: ['am', 'pm']
  };
};
