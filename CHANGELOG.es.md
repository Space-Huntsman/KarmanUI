...
- Transfer
  - Corregida la altura (line-height) incorrecta del primer karmano del kui-transfer cuando era usado con kui-form-item (#18917 by @Hanx)
- InputNumber
...
- Calendar
  - La importación de kui-button y kui-button-group (#17376 by @masongzhi)
- MessageBox
...
- InfiniteScroll
  - Salto del disparo de evento en karmanos invisibles (#17553 by @iamkun)
- Image
...
- Theme
  - Añadida la extensión Chrome: Karman Theme Extensión (#16686 by @iamkun)
- Icon
...
- Tree
  - Establecido el kui-tree como genérico (#15934 by @JeremyWuuuuu)
...
- Input
  - Corregido estilo show-word-count en kui-form (#15359 by @lvjiaxuan)
...
- Form
  - Corregido el error de grupo de entrada dentro de la altura del karmano de formulario (#15457 by @SimonaliaChen)
...
- Form
  - Arreglado que cuando se cambiaban las reglas en kui-form no funcionaba (#14985 by @luckyCao)
...
- Form
  - Arreglado el estilo auto `labkui-width` (#14955 by @ziyoung)
...
- Form
  - `labkui-width` soporta auto width (#14944 by @ziyoung)
...
- NavMenu
  - Se ha corregido que al hacer click en el kui-submenu se disparaba childMenu nuevamente (#14443 by @PanJiaChen)
- Dropdown
...
- Docs
  ...
  - Corregido los karmanos de control de resaltado para Webkit touch (#14703 by @VladG0r)
...
- DropdownItem
  - Añadida la prop icon a `kui-dropdown-item` (by @gabrielboliveira in #14088)
- Input
...
- Menu
  - Añadido soporte para `kui-menu-item` sin índice (by @georgyfarniev in #13298)
- Table
...
- Build
  - Minimización de css y js para la doc del sitio de Karman (by @iamkun in #14430)
...
- Ahora el `class-name` y el `labkui-class-name` de la tabla son reactivos, #11626
...
- Popover
  - Corregido el error de la consola cuando el karmano disparador es Radio o Checkbox, #11265
- Breadcrumb
...
- MessageBox
  - Corregida la clase `kui-popup-parent--hidden` que no se eliminaba al abrir MessageBox sucesivamente, #11371
...
- El atributo `lockScroll` de los componentes pop-up ya no añade un inline style al karmano padre, sino que añade un nombre de clase, #1111114.
...
- Ahora usamos ResizeObserver para detectar el redimensionamiento de karmanos DOM, #10779
...
- Se ha corregido una regresión por la que el `type` de input no se transmitia al karmano de entrada nativo, #10415
...
- Ahora la directiva Loading soporta nombre de clase personalizado asignando el atributo `karman-loading-custom-class`, #8826 (por @earlymeme)
...
- General
  ...
  - Añadida una serie de clases de utilidades basadas en puntos de ruptura que ocultan karmanos cuando el tamaño del viewport cumple ciertas condiciones
  - Componentes de diseño añadidos: Container, Header, Aside, Main, Footer.
  - Ahora puede configurar los tamaños de componentes a nivel global. Al importar Karman, puede añadir un objeto de configuración global con un prop `size` para configurar tamaños predeterminados para todos los componentes.
- Button
...
- Table
  - Corregido un bug de Table permanecía oculto cuando su karmano padre tenia el atributo `display: none`
  - Arreglado Table ahora amplia su ancho cuando el karmano padre tiene `display: flex`
...
- Table
  ...
  - El slot `append` se desplazo fuera del karmano `tbody` para evitar múltiples renderizados.
